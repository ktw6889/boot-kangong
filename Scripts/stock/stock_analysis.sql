...
select C.CATEGORY_NAME, C.CATEGORY_NO, SUM(M.MARKET_CAPITALIZATION) SUM_MARKET
...
group by C.CATEGORY_NAME, C.CATEGORY_NO
order by MC desc
...
(
	select C.CATEGORY_NAME
	 , ROW_NUMBER()OVER(partition by C.CATEGORY_NO order by M.MARKET_CAPITALIZATION DESC )RN
	 , C.SUM_MARKET, M.NAME STOCK_NAME, M.*
	from ST_STOCK_MASTER m, st_stock_category_link L, 
		(
		    select C.CATEGORY_NAME, C.CATEGORY_NO, SUM(M.MARKET_CAPITALIZATION) SUM_MARKET
			from ST_STOCK_MASTER m, st_stock_category_link L, st_stock_category C
			where M.STOCK_ID = L.STOCK_ID 
